...
for (let i of document.querySelectorAll(".aWrap")) {
    ...
    i.audio.onplay = () => (i.aPlay.classList.add("playing"));
    i.audio.onpause = () => (i.aPlay.classList.remove("playing"));
    ...
    i.audio.oncanplaythrough = () => {
        ...
        if (i.aVolume) { i.aVolume.disabled = false; }
        i.aSeek.disabled = false;
    };
    i.audio.onwaiting = () => {
        i.aPlay.disabled = true;
        if (i.aVolume) { i.aVolume.disabled = true; }
